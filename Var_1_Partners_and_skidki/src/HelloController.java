...
//          Загрузка данных из БД в боксы
    ...
    private void updateHBoxStyles() {
        for (javafx.scene.Node node : contentBox.getChildren()) {
            node.setStyle("-fx-border-color: black; -fx-padding: 10; -fx-background-color: #F4E8D3"); // Возвращаем обычный стиль
        }
    }
        ...
        partnerBox.setOnMouseClicked(event -> {
            if (event.getButton() == MouseButton.PRIMARY) {
                if (lastClickedPartnerBox != null && lastClickedPartnerBox != partnerBox) {
                    ...
                    lastClickedPartnerBox.setStyle("-fx-border-color: black; -fx-padding: 10; -fx-background-color: #F4E8D3");
                }
                // Инвертируем выделение текущего элемента
                if (partnerBox.getStyle().contains("-fx-background-color: #ADD8E6")) {
                    partnerBox.setStyle("-fx-border-color: black; -fx-padding: 10; -fx-background-color: #F4E8D3"); // Снимаем выделение
                    lastClickedPartnerBox = null; // Сбрасываем последний выбранный элемент
                } else {
                    partnerBox.setStyle("-fx-border-color: black; -fx-padding: 10; -fx-background-color: #ADD8E6"); // Добавляем выделение
